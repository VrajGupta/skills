...
Four related decisions made together for `vskills` v1:

- **Single agent target for v1.** `vskills` writes real content into the **install
...
- **Drift detected by content hash, not git.** The **manifest** records a hash per
  ...
  of a real three-way merge for v1 — on drift, `vskills` skips and warns rather than
...
`vskills` existed (exactly the state this repo was in prior to building this tool).
