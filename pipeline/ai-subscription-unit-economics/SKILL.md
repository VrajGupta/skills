...
## Step 1 â€” Cost per action, bottom-up
...
cost_per_action =
    ...
    + (cached_input_tokens Ã— cache_read_price)
...
- **Forgetting the system prompt and context** â€” often larger than the user's message, and paid on *every* turn.
...
- **Assuming a cache hit rate** you haven't measured. Prompt caching is a large lever â€” verify current pricing and TTL rather than recalling it.
- **Agentic loops.** One user action can be N model calls. Count N at the p50 *and* the p95.

Get current model prices from the source before computing â€” do not price from memory. (`claude-api` skill for Claude models.)

## Step 2 â€” Cost per user

```
cost_per_user_month = actions_per_month Ã— cost_per_action
```

Compute at three points â€” you're pricing a distribution, not an average:

| Cohort | Why it matters |
|---|---|
| **Median user** | Sets your typical margin |
| **p95 user** | Sets your real exposure |
| **Max user under current limits** | Your **worst case per subscriber** â€” if this exceeds the price, the plan is unbounded-loss by design |

The max-user number is the one people skip and the one that kills subscription AI products.

## Step 3 â€” The margin gate

```
gross_margin = (price âˆ’ cost_per_user_month) / price
```

Decide the floor **before** you fall in love with a price. Then solve the other direction:

```
breakeven_actions = (price Ã— (1 âˆ’ target_margin)) / cost_per_action
```
...
## Step 4 â€” Levers before raising price

In rough order of preference:

1. **Cheaper model for the cheap parts** â€” routing simple steps to a smaller model is usually the biggest single win.
2. **Prompt caching** for stable system prompts and long context.
3. **Trim context** â€” truncate history, summarize, retrieve instead of stuffing.
4. **Cap output tokens** â€” output is typically the expensive side.
5. **Deduplicate work** â€” cache results of identical requests.
6. **Batch** where latency permits.
7. **Then** adjust limits or price.

## Step 5 â€” Encode the result as invariants
...
- Cache hit rate assumed: H% â€” alert if it drops below Hâˆ’10
```

An unenforced cap is a wish. Each of these needs an enforcement point and a signal â€” see `invariant-evidence-review`.
...
- Live provider spend is **separately authorized** â€” measuring real costs may itself cost money. Ask first.
- State your assumptions (tokens per call, retry rate, cache hit rate) next to the numbers, so they can be checked later.

## Related

`part1` (invariant lock) Â· `invariant-evidence-review` Â· `controlled-ticket-delivery` Â· `claude-api`
