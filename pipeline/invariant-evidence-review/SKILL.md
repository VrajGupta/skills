...
- **Enforcement point** â€” the actual line/module in the **production path** that makes the invariant hold. Not the plan. Not a comment. A code location.
- **Proof** â€” a test that **fails** when the invariant is violated. If violating it keeps the suite green, there is no proof.
- **Production signal** â€” what would surface a violation in the wild: a metric, an alert, a log, a check. Optional for some invariants; its absence should still be stated.
...
- **Performance / resource budgets** â€” concrete numbers. "Fast" is not an invariant.
- **Failure and recovery per dependency** â€” down / slow / rate-limited / returning garbage. What does the user see? Retried, surfaced, or degraded?
- **Security & authorization boundaries** â€” who may do what, and the blast radius when it's wrong.
- **Privacy** â€” what data leaves the system, what gets logged, what's retained.
- **Permissions / tenancy** â€” cross-tenant access denied, and **tested explicitly**.
- **Data integrity** â€” uniqueness, referential rules, idempotency, migration reversibility.
- **UX / compatibility / ops** â€” where the plan named them.
...
| Test mocks the very component that enforces it | Tautology â€” proves the mock works |
...
| "Handled gracefully" with no observable | Unfalsifiable â€” cannot be reviewed |
...
1. List the invariants from the plan/ticket verbatim. Don't paraphrase â€” paraphrasing softens them.
2. For each, **grep to the enforcement point** in the production path.
3. For each, find the test â€” then **prove the test is real** by asking: what change would make it fail? If nothing obvious would, it's weak.
...
Findings:
1. <invariant> â€” <verdict> â€” evidence: <command output or file:line>
   Impact: ...
   Suggested ticket: <title> â€” verification: `<cmd>`
```

Verdicts: **enforced** (all three columns real) Â· **documented-only** (no enforcement point) Â· **unproven** (enforced but no failing-on-violation test) Â· **violated** (demonstrated broken).
...
`part3` Â· `part1` (locks the invariants) Â· `provider-integration-tdd` Â· `code review` workflows
