...
version: 1.0.0
description: Operator manual and paid-lessons list for the Linear pipeline — the GitHub Issues Sync duplicate disaster, silent state no-ops, exact-name requirements, state-vs-label confusion, and how the older GitHub triage label vocabulary (needs-triage, ready-for-agent, wontfix) maps onto Linear states. Use when diagnosing why a ticket did not move, when duplicates appear, when deciding whether a repo uses Linear states or GitHub labels, or when onboarding a new repo into the pipeline.
---

# linear-label-pipeline — operator manual & pitfalls
...
1. Linear ↔ GitHub Issues Sync was enabled workspace-wide.
2. Agents, following older GitHub-era triage habits, applied `ready-for-agent` labels to GitHub issues **#161–168**.
3. Sync interpreted the GitHub-side edits as new work and created **LUL-121–128** in Planned.
4. All eight had to be manually canceled.

Root cause: **dual-writing stage into two synced systems.** The label was not wrong as vocabulary — it was wrong as a *second write surface*.
...
Linear state mutations can return a **200 with full issue JSON** and change nothing — most often because:
...
`Agent Ready` ≠ `agent-ready` ≠ `Agent-Ready` ≠ `Ready for Agent`.
`Debugger Ready` ≠ `Debug Ready` ≠ `In Review`.
...
- **Workflow state** — canonical for pipeline stage. Drives the queues each role reads.
- **Labels** — descriptive tags (`bug`, `enhancement`, area tags). Useful, never authoritative for stage.
...
The engineering pack still speaks GitHub/label dialect. That dialect is **not obsolete** — it's the general tracker language for non-synced repos. On Linear-synced repos, overlay it:
...
```
qa            → conversational bug capture → issue
triage        → categorize / verify / grill / brief → ready-for-agent
to-prd        → structure the work as a spec
to-issues     → tracer-bullet tickets with blockers
part2         → build
part3         → check
```

Refactor variant: `request-refactor-plan` → tiny-commit plan as an issue.
Fog variant: `wayfinder` → investigation map until `part1` is sane.

Setup once per repo: `setup-matt-pocock-skills` wires tracker config, label vocabulary, and domain doc layout. If it was never run, **local markdown tickets are the safe fallback** — never invent a tracker.

## 7. Diagnostic table — "the ticket didn't move"
...
| OAuth failures mid-run | Token expired | Re-auth once, then stop — don't thrash |

## 8. Model / tooling notes

- Pin the model per role deliberately; a cheap model on the debugger defeats maker≠checker.
- MCP Linear tools and `gh` are different authorities — never let a `gh` convenience call substitute for a Linear write.
