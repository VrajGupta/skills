...
are independent top-level parent sessions for coder, debugger, and grader. Each
...
4.5/Pi via OpenRouter for grader.

The stage boundary is carried by the GitHub Project item, GitHub issue/PR
artifacts, and handoffs. Keep the stages serial and preserve a fresh grader context.
...
|---|---|---|---|
| `planner` | Planned | `github-projects-pipeline` → `part1` (+ `batch-grill-me`) | creates GitHub issues in Planned; promotes unblocked project items to Agent Ready |
| `coder` | Agent Ready (incl. scope/test bounce returns) | `github-projects-pipeline` → `part2` (+ `ticket-implementation-tdd`) | Agent Ready → Coding → Debugger Ready |
| `debugger` | Debugger Ready (incl. correctness bounce returns) | `github-projects-pipeline` → `part3` (+ `invariant-evidence-review`) | Debugger Ready → Debugging → Grading Ready |
| `grader` | Grading Ready | `github-projects-pipeline` → `part4` (+ grading rubric) | Grading Ready → Grading → Done or classified bounce |
...
`profile-gated-delivery` · `github-projects-pipeline` · `part1` · `part2` · `part3`
