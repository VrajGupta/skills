...
## Step 1 â€” Smell test for false progress
...
| Repeated retries produce **identical** failure output | No new information is being generated â€” burn without learning |
...
## Step 2 â€” Re-read reality, in this order

Do not touch code yet.

1. The ticket: current state, full body, **all comments** (bounces especially).
2. The plan doc it points at.
3. The **production entry point** â€” does real traffic actually reach the code under test?
4. The gate: run it. Right now. Record the real output.
5. `git log --oneline -10` and `git status -sb` â€” what actually landed?
...
**A. Implementation defect** â€” the design is sound, the code is wrong. The ticket *can* land as written.
â†’ Recover with `part2` discipline: lock the gate, reproduce, test-first, smallest fix, re-gate. Apply a repair budget (default five meaningful attempts), then escalate to B.

**B. Structural blocker** â€” the ticket *cannot* land as specified. A prerequisite doesn't exist, the seam is wrong, the plan assumed something false, or the invariant needs a boundary that isn't there.
â†’ **Do not keep burning the child.** Instead:
...
Misclassifying B as A is the single most expensive mistake here â€” it burns budget forever on a ticket that was never landable.

## Step 4 â€” Recovery principles
...
## Step 5 â€” Write the recovery record
...
State: <unchanged | moved X â†’ Y, readback confirmed>
Next owner: <role>
```
...
- Treating a timeout as a total loss â€” assess what landed first (`shared-worktree-safety`).

## Related

`linear-pipeline` Â· `linear-label-pipeline` Â· `part2` Â· `part3` Â· `shared-worktree-safety` Â· `invariant-evidence-review`
