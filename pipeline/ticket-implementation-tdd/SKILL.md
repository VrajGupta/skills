...
## Step 1 â€” Lock the gate BEFORE editing
...
2. If it doesn't run (missing harness, wrong path, no such script) â€” that's a **ticket defect**. Fix the command with the user's agreement *before* treating it as the done-definition, and note the correction on the ticket.
3. If the ticket has no command, derive one and get it agreed. Do not invent a private definition of done.

Once locked, the gate does not move to accommodate your implementation. If the gate is wrong, you change it deliberately and say so â€” you never quietly relax it.

## Step 2 â€” Red at the highest meaningful seam
...
- The test must exercise the seam **production traffic actually uses**. A test against a path the app never takes proves nothing â€” this is the single most common false-green in this pipeline.
...
## Step 3 â€” Green with the smallest correct change

- Smallest change that makes the test pass honestly.
- **No opportunistic rewrites** tangled into the ticket.
- Preserve the codebase's domain vocabulary and module boundaries â€” if the ticket forces you to violate one, that's a bounce-worthy finding, not a thing to quietly do.
- Scope is **one ticket**. Discoveries outside it become follow-up tickets, not extra diff.

## Step 4 â€” Provider-safe seams

For anything crossing a network or costing money:

- Put the provider behind a **seam the production path uses too** â€” inject the client; don't monkeypatch the module under test.
...
## Step 5 â€” Refactor under green

Only with the gate green. Behavior unchanged. Re-run after.

## Step 6 â€” Native checks + final gate

In this order:

```
typecheck  â†’  lint  â†’  build (if applicable)  â†’  the locked gate  â†’  proportional broader suite
```
...
Output:
<verbatim>
Native checks: typecheck âœ… lint âœ… build âœ…
Broader: `<cmd>` â†’ pass
Commit: <sha>
...
- Unrelated dirty files are sacred â€” stage only ticket-owned paths.

## Related

`part2` Â· `tdd` Â· `provider-integration-tdd` Â· `invariant-evidence-review` Â· `shared-worktree-safety`
